...
export class TestPlace extends NetworkedScene {
	...
	//Переменные для шляп
	private currentHatIndex: number = 0;
	private hatSprites: Phaser.GameObjects.Sprite[] = [];
	private availableHats: string[] = [];
	private hatVisible: boolean = true;
	...
	onPreload() {
		...
		this.load.image(ASSET_KEYS.HAT_1, ASSET_URLS[ASSET_KEYS.HAT_1]);
		this.load.image(ASSET_KEYS.HAT_2, ASSET_URLS[ASSET_KEYS.HAT_2]);
		this.load.image(ASSET_KEYS.HAT_3, ASSET_URLS[ASSET_KEYS.HAT_3]);
	}
	...
	onCreate() {
		...
		this._initInput();
		this._initCamera();
		this._initHats(); // Инициализируем систему шляп
	}
	...
	heartbeat(_: number, delta: number): void {
		...
		this._handleHatInput(); // Обрабатываем ввод для шляп
	}
	//================================================================
	...
	private _initInput(): void {
		this.keys = this.input.keyboard?.addKeys('W,A,S,D,H,J,K') as {
	...
	// СИСТЕМА ШЛЯП
	//================================================================
	private _initHats(): void {
		// Список доступных шляп
		this.availableHats = [
			ASSET_KEYS.HAT_1,
			ASSET_KEYS.HAT_2,
			ASSET_KEYS.HAT_3,
		...
		// Создаем спрайты для всех шляп, но изначально скрываем их
		this.availableHats.forEach((hatKey, index) => {
			const hat = this.add.sprite(
				this.player.x,
				this.player.y - 20,
				hatKey,
			);
			hat.setDepth(11); // Шляпа поверх игрока
			hat.setVisible(index === this.currentHatIndex); // Показываем только текущую шляпу
			hat.setScale(0.025); // Настраиваем размер под персонажа

			this.hatSprites.push(hat);
		});
	}

	private _handleHatInput(): void {
		// Клавиша H - следующая шляпа
		if (Phaser.Input.Keyboard.JustDown(this.keys.H)) {
			this._nextHat();
		}

		// Клавиша J - предыдущая шляпа
		if (Phaser.Input.Keyboard.JustDown(this.keys.J)) {
			this._previousHat();
		}

		// Клавиша K - скрыть/показать шляпу
		if (Phaser.Input.Keyboard.JustDown(this.keys.K)) {
			this._toggleHat();
		}
	}

	private _nextHat(): void {
		this.currentHatIndex =
			(this.currentHatIndex + 1) % this.availableHats.length;
		this._updateHatVisibility();
	}
	private _previousHat(): void {
		this.currentHatIndex =
			(this.currentHatIndex - 1 + this.availableHats.length) %
			this.availableHats.length;
		this._updateHatVisibility();
	}

	private _toggleHat(): void {
		this.hatVisible = !this.hatVisible;
		this._updateHatVisibility();
	}

	private _updateHatVisibility(): void {
		this.hatSprites.forEach((hat, index) => {
			hat.setVisible(this.hatVisible && index === this.currentHatIndex);
		});
	}

	private _updateHatPosition(): void {
		if (!this.hatSprites[this.currentHatIndex] || !this.hatVisible) return;

		const hat = this.hatSprites[this.currentHatIndex];

		// Проверяем, есть ли физическое тело у игрока
		if (this.player.body) {
			const body = this.player.body as Phaser.Physics.Arcade.Body;
			hat.x = body.x + body.width / 2;
			hat.y = body.y - 28;
		} else {
			// Fallback на обычные координаты
			hat.x = this.player.x;
			hat.y = this.player.y - 16;
		}

		hat.flipX = this.player.flipX;
	}
